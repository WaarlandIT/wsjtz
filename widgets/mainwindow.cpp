  ...
  m_block_udp_status_updates {false}
{
  ...
  setWindowTitle (program_title () +  " (WSJT-Z MOD v1.10 by SQ9FVE)") ;
        ...
        if (m_unfilteredView && m_unfilteredView->isVisible()) {
          ...
          if (!isFiltered || for_us)
          {
              ...
              ui->decodedTextBrowser->displayDecodedText(decodedtext1,m_baseCall,m_mode,m_config.DXCC(),
                                                         ...
                                                         haveFSpread, fSpread, ui->cbCQonlyIncl73->isChecked(), m_config.colourAll(), distance, state);
...
//### I think this is where we are preventing Hounds from spotting Fox ###
      // Z
      if (!(isFiltered && m_config.autoCQfiltering()))
...
void MainWindow::on_logQSOButton_clicked()                 //Log QSO button
{
    ...
    if (!m_hisCall.size () && (ui->cbAutoCQ->isChecked() || ui->cbAutoCall->isChecked())) {
        log("on_logQSOButton_clicked: m_hisCall is empty. Exiting.");
        clearDX();
        m_inQSOwith="";
        return;
    }
  ...
  if(grid=="....") grid="";
  // Z
  if (ui->ci_grid->text().length() > grid.length())  grid = ui->ci_grid->text();
...
bool MainWindow::callsignFiltered(DecodedText dt)
{
    ...
    bool is_73 = (message_words.size() == 4 && (message_words[3] == "73" || message_words[3] == "RR73"));
    ...
    if (!message_words[1].startsWith("CQ") && ui->cb_ignoreCQTarget->currentIndex() == 3) return true;

    if( message_words.size() > 2 && ( ui->cb_ignoreCQTarget->currentIndex() > 0 || ui->cb_filter_CQDX_Continent->currentIndex() > 0) && message_words[1].startsWith("CQ")) {
        QString w0 = message_words[0];
        QString w1 = message_words[1];
        QString w2 = message_words[2];
...
void MainWindow::switchBands() {
   ...
   if (idx >= 0 && idx < bands.size()-1) {
    ...
    if (row >= 0) {
        ...
        log("switchBands: Changed band to: " + newBand);
    }

    log("switchBands: EXIT");
